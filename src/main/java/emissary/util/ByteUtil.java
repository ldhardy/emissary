...
/**
 * Common place for the logic to glue byte arrays back together. This is
 * error-prone and shouldn't be thought about any more than necessary
 */
public class ByteUtil {
	 ...
	 */
	@SuppressWarnings("InconsistentOverloads")
	public static byte[] glue(byte[] a, int astart, int aend, byte[] b, int bstart, int bend, byte[] c, int cstart,
			int cend) {
		int alen = aend - astart + 1;
		int blen = bend - bstart + 1;
	...
	/**
	 * Given a byte-array and a start offset, return a string of the bytes between
	 * the start position and a carriage return byte. In essence, this is grabbing a
	 * line of input where the byte array is composed of several lines of input.
	 * 
	 * @param data The byte array of input data.
	 * @param pos  The initial start offset.
	 * @return A string created from the bytes found from the start offset to the
